	...
	for(uint32_t i = 0; i < data->nodes_count; i++)
	{
		...
		for(uint32_t t = 0; t < mesh->primitives_count; t++)
		{
			...
			if(prim->material->pbr_metallic_roughness.base_color_texture.texture)
			{
				...
				texsize = prim->material->pbr_metallic_roughness.base_color_texture.texture->image->buffer_view->size;  
				UploadTextureGPU_2Dmipped(CreateInfo.TexInfo.TexArray[0], DataBaseAddr, absbufferoffset, texsize);
			}
			if(prim->material->pbr_metallic_roughness.metallic_roughness_texture.texture)
			{
				...
				UploadTextureGPU_2Dmipped(CreateInfo.TexInfo.TexArray[1], DataBaseAddr, absbufferoffset, texsize);
			}
			if(prim->material->normal_texture.texture)
			{
				...
				UploadTextureGPU_2Dmipped(CreateInfo.TexInfo.TexArray[2], DataBaseAddr, absbufferoffset, texsize);
			}
